...
static int set_curr_elem_found = 0;
static int set_pid_map_size = 0;

static struct proc_dir_entry *set_file;

// TODO: cleanup for a particular PID.
...
static ssize_t set_file_write(struct file *file, const char __user *user_buff, size_t count, loff_t *pos) {
    ...
    // TODO: first check if the current pid is a new process or existing process
    // If it is a new process, find a position for it. If no position, return error
